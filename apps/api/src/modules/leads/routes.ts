...
import { LeadStatus, UpdateLeadRequestDto } from "@elysium-crm/shared-types/dto/lead";

import { AuthenticatedRequest, requireAuth } from "../../middleware/auth";
import { getLeadById, listLeads, updateLead } from "./service";
...
leadsRouter.get("/", async (req, res, next) => {
  ...
  const validStatuses = new Set<string>(["ALL", ...Object.values(LeadStatus)]);
...
leadsRouter.put("/:id", async (req, res, next) => {
  ...
  if (payload.status && !Object.values(LeadStatus).includes(payload.status)) {
    res.status(400).json({ error: "Invalid lead status" });
    return;
  }

  try {
    const lead = await updateLead(user.organizationId, req.params.id, payload);
    res.json(lead);
  } catch (err: any) {
    if (err?.status === 404) {
      res.status(404).json({ error: "Lead not found" });
